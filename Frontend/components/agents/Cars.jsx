...
export default function Cars() {
  ...
  const [allMappedCars, setAllMappedCars] = useState([]);
  const [filtered, setFiltered] = useState([]);

  // 1. Map the backend data when it arrives
  useEffect(() => {
    if (backendListings) {
      ...
      setAllMappedCars(mapped);
    }
  }, [backendListings]);

  // 2. Filter the dynamically mapped cars when the type changes
  useEffect(() => {
    if (selectedType === "All") {
      setFiltered(allMappedCars);
    } else {
      setFiltered(allMappedCars.filter((el) => el.type === selectedType));
    }
  }, [selectedType, allMappedCars]);

  console.log("Mapped Cars:", allMappedCars);
              ...
              {loading ? (
                ...
                <div className="list-car-grid-4 gap-30">
                  {filtered.slice(0, 8).map((car, index) => (
                    <div key={car.id || index} className="box-car-list hv-one">
