...
export default function Cars2() {
  ...
  // 3. Map the data when it arrives
  useEffect(() => {
    if (backendListings) {
      const mapped = backendListings.map((item) => {
        let dbCategory = "In Stock";
        if (item.status === "in_transit") dbCategory = "In Transit";
        if (item.status === "on_order") dbCategory = "On Order";
  ...
  // Filter the dynamically mapped cars
  const filteredCars = dynamicCars.filter((elm) =>
    elm.category.includes(selectCategory),
  );
                                ...
                                {/* <div className="days-box flex justify-space align-center">
                                  {/* <div className="img-author">
                                    <Image
                                      ...
                                      alt={car.authorName}
                                      src={car.authorImage}
                                      width={40} // Scaled down the avatar width to normal size
                                      height={40}
                                      style={{
                                        borderRadius: "50%",
                                        objectFit: "cover",
